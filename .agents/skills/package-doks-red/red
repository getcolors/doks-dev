#!/usr/bin/env bun
// Native DOKS launcher. Desired state lives in colors.yml; the package
// owns validation and lifecycle steps. Copied launchers resolve immutable pins.
...
//
// "package-doks-red" is null before this repository has been pushed and
...
// rewrites null to "github:getcolors/doks#<sha>" in place.
//
...
// directory, which would halt Bun's upward resolution of `package-doks-red`
// and break the development symlink at red/red.
const PINS = {
  "package-doks-red": "github:getcolors/doks#8120674e81d88827c4e8b6eaa899b0c5fbaf829c",
  ...
  "red": "github:getcolors/red#aa223dd1eae8891633c6fda4bd6f55cb428cb4f8",
};
...
// add-deps. DOKS_LIB_ROOT overrides everything with a working tree: point it
// at a doks checkout (its root, or its `red/` directory) whose dependencies
 ...
 *
 * The payload's canonical home is skills/package-doks-red/ inside the doks
...
function checkoutRedDir() {
  ...
  return manifest?.name === "package-doks-red-dev" ? candidate : null;
}
...
function installToCache() {
  ...
  const root = join(home, "package-doks-red");
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: "package-doks-red-cache", private: true, dependencies: PINS }, null, 2)}\n`,
  );
...
let doks;
try {
  doks = await import("package-doks-red");
} catch (err) {
  ...
  const libRoot = process.env.DOKS_LIB_ROOT;
  ...
  if (libRoot) {
    ...
    doks = await importWorkingTree(overrideDir, "DOKS_LIB_ROOT");
    ...
    doks = await importWorkingTree(redDir, "checkout");
  } else if (!PINS["package-doks-red"]) {
    console.error(
      "red: this launcher carries no doks pin.\n" +
    ...
    // not to a path.
    doks = await import(Bun.resolveSync("package-doks-red", dir));
  }
}

export const run = doks.run;
if (import.meta.main) await doks.exec();
